...
static void duk__console_reg_vararg_func(duk_context* ctx,
                                         duk_c_function func,
                                         const char* name,
                                         duk_uint_t flags) {
  ...
  duk_def_prop(ctx, -3,
               DUK_DEFPROP_HAVE_VALUE |
                   DUK_DEFPROP_FORCE); /* Improve stacktraces by displaying
                                          function name */
...
void duk_console_init(duk_context* ctx, duk_uint_t flags) {
  ...
  /* If both DUK_CONSOLE_STDOUT_ONLY and DUK_CONSOLE_STDERR_ONLY where
   * specified, just turn off DUK_CONSOLE_STDOUT_ONLY and keep
  ...
  flags = flags_orig;
  if (!(flags & DUK_CONSOLE_STDOUT_ONLY) &&
